...
export const metadata: Metadata = {
  title: "Digital Marketing Services Montreal | Omnivision Design",
  description:
    "Omnivision Design offers web design, SEO, Google Ads, social media marketing, email marketing, AI services and more. Professional digital marketing in Montreal.",
};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {allServices.map((s) => (
            ...
            >
              ...
              <Link href={s.href}>
                <h2 className="font-display text-2xl font-bold text-white mb-6 group-hover:text-brand-amber transition-colors">
                  {s.category}
                </h2>
              </Link>
              <ul className="space-y-3">
                {s.items.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className="text-sm text-white/60 hover:text-brand-amber transition-colors flex items-center gap-2"
                    >
