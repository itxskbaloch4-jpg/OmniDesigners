...
export const metadata: Metadata = {
  title: "Our Clients | Omnivision Design Montreal",
  description:
    "Omnivision Design has served hundreds of clients across Montreal and Canada, from startups to public companies including Telus, Air Canada, Aeroplan, and Airbnb.",
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-white/60 text-lg max-w-2xl mx-auto">
            From startups to public companies — businesses across Canada and the USA trust Omnivision
            Design to grow their online presence.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {items.map((client) => (
            <div
              key={client.name}
              className="flex items-center justify-center p-6 bg-white/5 border border-white/10 rounded-2xl hover:border-brand-amber/40 hover:bg-white/10 transition-all duration-300 group aspect-[3/2]"
            >
              <Image
                src={client.logo}
                alt={client.name}
                width={140}
                height={70}
                className="object-contain max-h-14 filter grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300"
              />
            </div>
          ))}
        </div>
