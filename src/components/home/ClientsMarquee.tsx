...
export default function ClientsMarquee() {
  ...
  return (
    <section
      ref={ref}
      className="relative py-24 bg-brand-brown overflow-hidden"
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="flex gap-12 animate-marquee" style={{ width: "max-content" }}>
          {[...clients, ...clients].map((client, i) => (
            <div
              key={i}
              className="flex-shrink-0 flex items-center justify-center w-40 h-20 px-4 bg-white/5 border border-white/10 rounded-xl hover:border-brand-amber/30 transition-all duration-300 group"
            >
              <Image
                src={client.logo}
                alt={client.name}
                width={120}
                height={60}
                className="object-contain max-h-12 filter grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300"
              />
            </div>
          ))}
        </div>
