  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-brand-brown/95 backdrop-blur-xl shadow-2xl border-b border-brand-amber/20"
          : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex-shrink-0 group">
            ...
            >
              <Image
                src="https://www.omnivisiondesign.com/wp-content/themes/omnivision/img/omnivision-logo-white.svg"
                alt="Omnivision Design Logo"
                width={180}
                height={50}
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-white/90 hover:text-brand-amber transition-colors duration-200 rounded-lg hover:bg-white/5"
                >
                  {item.label}
                  {item.children && (
                    ...
                    >
                      {item.children.map((child) => (
                        <Link
                          key={child.label}
                          href={child.href}
                          className="block px-4 py-3 text-sm text-white/80 hover:text-brand-amber hover:bg-brand-amber/10 transition-all duration-200 border-b border-white/5 last:border-0"
                        >
                          {child.label}
                        </Link>
                      ))}
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="px-6 py-2.5 bg-brand-amber text-brand-brown font-bold text-sm rounded-full hover:bg-brand-amber-light transition-all duration-300 amber-glow"
            >
              Request a Quote
            </motion.a>
          </div>
          ...
          <button
            ...
            className="lg:hidden p-2 text-white hover:text-brand-amber transition-colors"
          >
              ...
              {navItems.map((item) => (
                <div key={item.label}>
                  <Link
                    ...
                    className="block px-4 py-3 text-white hover:text-brand-amber hover:bg-white/5 rounded-lg transition-colors font-medium"
                  >
                    {item.label}
                  </Link>
                  {item.children && (
                    <div className="ml-4 space-y-1">
                      {item.children.map((child) => (
                        <Link
                          ...
                          href={child.href}
                          onClick={() => setMobileOpen(false)}
                          className="block px-4 py-2 text-sm text-white/70 hover:text-brand-amber transition-colors"
                        >
                          {child.label}
                        </Link>
                      ))}
